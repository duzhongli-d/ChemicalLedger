...
async def upload_source(
    ...
    # Use provided source_type if valid, otherwise detect from MIME
    if source_type and source_type.upper() in [t.value for t in ResearchSourceType]:
        source_type_enum = ResearchSourceType(source_type.upper())
    else:
        # Determine source type from MIME type
        content_type = file.content_type or ""
        if content_type == "application/pdf" or file.filename.endswith(".pdf"):
            source_type_enum = ResearchSourceType.PDF
        elif content_type.startswith("text/") or file.filename.endswith((".txt", ".md", ".csv")):
            source_type_enum = ResearchSourceType.TEXT
        elif content_type.startswith("video/"):
            source_type_enum = ResearchSourceType.VIDEO
        elif content_type.startswith("audio/"):
            source_type_enum = ResearchSourceType.AUDIO
        else:
            source_type_enum = ResearchSourceType.TEXT

    # Save file
    file_content = await file.read()
    relative_url, file_size, extra_data = research_service.save_uploaded_file(
        file_content, file.filename or "upload", source_type_enum
    )
...
async def generate_learning_guide(
    ...
    # Prepare source context (include TEXT content when available)
    source_info = []
    for s in sources:
        info = {"name": s.file_name or "Untitled", "type": s.source_type, "url": s.file_url}
        if s.source_type == ResearchSourceType.TEXT and s.extra_data:
            content = s.extra_data.get("content", "")
            if content:
...
async def generate_mindmap(
    ...
    source_info = []
    for s in sources:
        info = {"name": s.file_name or "Untitled", "type": s.source_type}
        if s.source_type == ResearchSourceType.TEXT and s.extra_data:
            content = s.extra_data.get("content", "")
            if content:
                info["content"] = content
